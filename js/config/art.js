////////// COFFEE /////////////

Background("coffeehouse","img/backgrounds/coffeehouse.png");
Background("coffeehouse_2","img/backgrounds/coffeehouse_2.png");
Background("shop","img/backgrounds/shop.png");
Background("outro_tw","img/backgrounds/outro_tw.png");

Still("lab_table","img/stills/lab_table.png");
Still("coffee_nicky_still","img/stills/coffee_nicky_still.png");
Still("coffee_nicky_still_2","img/stills/coffee_nicky_still_2.png");
Still("coffee_nicky_drink","img/stills/coffee_nicky_drink.png");
Still("coffee_nicky_throw","img/stills/coffee_nicky_throw.png");

Still("me_bad","img/stills/me_bad.png");
Still("me","img/stills/me.png");
Still("me_good","img/stills/me_good.png");
Still("me_bye","img/stills/me_bye.png");
Still("me_write","img/stills/me_write.png");

Sprite("cup_steam",{
	src: "img/sprites/cup_steam.png",
	width:80, height:80,
	frames:4, fps:2
});

////////// BEDROOM /////////////

Background("bedroom","img/backgrounds/bedroom.png");
Background("bedroom_2","img/backgrounds/bedroom_2.png");
Background("bedroom_3","img/backgrounds/bedroom_3.png");
Still("friend","img/stills/friend.png");
Still("building","img/stills/building.png");

Still("phone_b","img/stills/phone_black.png");

Background("bedroom_us_1","img/stills/bedroom_us_1.png");
Background("bedroom_us_2","img/stills/bedroom_us_2.png");
Background("bedroom_punch","img/stills/bedroom_punch.png");
Still("school_stand","img/stills/school_stand.png");
Background("school_alone","img/stills/school_alone.png");

Still("friend_2","img/stills/friend_2.png");
Still("city_arm","img/stills/arm_withfriend.png");
Still("gohome_alone","img/stills/gohome_alone.png");

Sprite("bedroom_light_1",{
	src: "img/sprites/bedroom_light_1.png",
	width:362, height:362,
	frames:4, fps:2
});

Sprite("bedroom_light_2",{
	src: "img/sprites/bedroom_light_2.png",
	width:362, height:362,
	frames:4, fps:2
})

Sprite("bedroom_2_light",{
	src: "img/sprites/bedroom_2_light.png",
	width:362, height:368,
	frames:4, fps:2
})
////////// DINNER /////////////

Background("dinner","img/backgrounds/dinner.png");

Still("dinner_table_2","img/stills/dinner_table_2.png");
Still("dinner_table_3","img/stills/dinner_table_3.png");

Still("dinner_nicky_sit","img/stills/dinner_nicky_sit.png");
Still("dinner_nicky_defiant","img/stills/dinner_nicky_defiant.png");
Still("dinner_nicky_outrage","img/stills/dinner_nicky_outrage.png");
Still("dinner_nicky_punched","img/stills/dinner_nicky_punched.png");
Still("dinner_punch_arm","img/stills/dinner_punch_arm.png");
Still("dinner_standing","img/stills/dinner_standing.png");

Still("mom_stand","img/stills/mom_stand.png");
Still("mom_sit","img/stills/mom_sit.png");

Still("mom_sitting","img/stills/mom_sitting.png");
Still("mom_cry","img/stills/mom_sad.png");
Still("mom_grab","img/stills/mom_grabbing.png");


Still("dad_anger","img/stills/dad_anger.png");
Still("dad_come","img/stills/dad_come.png");
Still("dad_serious","img/stills/dad_serious.png");
Still("dad_threat","img/stills/dad_threat.png");

Still("clock_1855","img/stills/clock_1855.png");
Still("clock_1900","img/stills/clock_1900.png");
Still("clock_1910","img/stills/clock_1910.png");
Still("clock_1920","img/stills/clock_1920.png");
Still("clock_1930","img/stills/clock_1930.png");
Still("clock_1940","img/stills/clock_1940.png");
Still("clock_1950","img/stills/clock_1950.png");
Still("clock_2000","img/stills/clock_2000.png");

Sprite("clock_ticking",{
	src: "img/sprites/clock_ticking.png",
	width:50, height:100,
	frames:4, fps:2
});
Sprite("clock_meowing",{
	src: "img/sprites/clock_meowing.png",
	width:50, height:100,
	frames:4, fps:10
})
